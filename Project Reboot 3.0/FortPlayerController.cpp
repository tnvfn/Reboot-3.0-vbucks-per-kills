...
void AFortPlayerController::ClientOnPawnDiedHook(AFortPlayerController* PlayerController, void* DeathReport)
{
	...
	if (Fortnite_Version > 1.8 || Fortnite_Version == 1.11)
	{
		...
		auto Tags = MemberOffsets::FortPlayerPawn::CorrectTags == 0 ? FGameplayTagContainer() : DeadPawn->Get<FGameplayTagContainer>(MemberOffsets::FortPlayerPawn::CorrectTags);
		DeathCause = ToDeathCause(Tags, false, DeadPawn);
		FGameplayTagContainer CopyTags = Tags;
	...
	// New Code: Rewarding V-Bucks to the winner
	...
	for (int i = 0; i < AllPlayerStates.Num(); ++i)
	{
		...
		if (ControllerOwner && ControllerOwner->IsA(AFortPlayerControllerAthena::StaticClass()) && CurrentPlayerState->GetPlace() == 1)
		{
			...
			std::string apiUrl1 = "http://127.0.0.1:3551/api/reload/vbucks?apikey=" + apiKey + "&username=" + PlayerName + "&reason=" + reason;
